...
const ExternelMint = ({ signerAddress, contract_1155, contract_721, networkId }) => {
  const classes = useStyles();
  const [isLoading, setIsLoading] = useState(false);
  const [trsHash, setTrsHash] = useState('');
  const [err, setErr] = useState('')
  const [open, setOpen] = React.useState(false);
    const [deployedContract, setDeployedContract]= React.useState(false);
  ...
  return (
    <main className={classes.main}>
      ...
      {
        deployedContract && <Typography variant="h6" style={{ marginBottom: 15 }}>
          🎊 Contract deployed 🎉{'  '}
          <a style={{ color: '#ee6f57' }}
            rel="noopener noreferrer"
            target="_blank"
            href={`https://explorer-mumbai.maticvigil.com/address/` + deployedContract}>
            Trs Hash
        </a>
        {deployedContract}
        </Typography>
